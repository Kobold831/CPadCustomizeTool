...
public class DownloadEventListenerList {
	...
	public void addEventListener(DownloadEventListener downloadEventListener) {
		listeners.add(downloadEventListener);
	}

	public void downloadCompleteNotify(int reqCode) {
		for (DownloadEventListener listener : listeners) {
			listener.onDownloadComplete(reqCode);
		}
	}

	public void downloadErrorNotify(int reqCode) {
		for (DownloadEventListener listener : listeners) {
			listener.onDownloadError(reqCode);
		}
	}

	public void connectionErrorNotify(int reqCode) {
		for (DownloadEventListener listener : listeners) {
			listener.onConnectionError(reqCode);
		}
	}

	public void progressUpdate(int progress, int currentByte, int totalByte) {
		for (DownloadEventListener listener : listeners) {
			listener.onProgressUpdate(progress, currentByte, totalByte);
		}
	}
}
