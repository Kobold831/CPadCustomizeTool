...
- 解像度の変更
- WebViewの起動
  - 内蔵ブラウザを起動します。セキュリティ保護のためJavaScriptによるファイルアクセス機能・非SSL接続は拒否しています。
- 学習両立モード
  - 緊急モード
...
付与される場合は以下のコマンドをADBで実行してください。

このコマンドを入力するとこのアプリにセキュア設定の変更を許可します。
...
デバイスオーナーは[Dhizuku](https://github.com/iamr0s/Dhizuku/releases)に設定することをおすすめします。

Dhizukuに設定した場合もこのアプリでデバイスオーナーの機能にアクセスできます。

デバイスオーナーの設定をするときはデバイスにアカウントがないか確認してください。
...
バグや不具合が繰り返し発生する場合は、お手数ですが[報告](https://github.com/Kobold831/CPadCustomizeTool/issues)のご協力をお願いいたします。
